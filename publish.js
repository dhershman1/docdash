/*global env: true */
'use strict';
var { capitalize, groupBy } = require('kyanite')
...
function buildNav (members) {
    const grouped = groupBy(x => x.category, members.globals)
    const categories = Object.keys(grouped)

    let nav = '<h2><a href="index.html">Home</a></h2>';
    const docdash = env && env.conf && env.conf.docdash || {};
    if (docdash.menu) {
    ...
    const globalNav = categories.reduce((acc, k) => {
        acc += `<h3>${capitalize(k)}</h3>`
        acc += grouped[k].reduce((html, fn) => {

            return html + `<li class="nav-item" data-cat="${fn.category}"><a href="functions.html#${fn.name}">${fn.name}</a></li>`
        }, '')

        return acc
    }, '')

    nav += `<ul class="nav-list">${globalNav}</ul>`
...
exports.publish = function (taffyData, opts, tutorials) {
    ...
    var globalUrl = helper.getUniqueFilename('functions');
    helper.registerLink('functions', globalUrl);
    ...
    if (members.globals.length) {
        generate('', env.conf.docdash.moduleName || 'Untitled', [{ kind: 'globalobj' }], globalUrl);
    }
